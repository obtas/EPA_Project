...
function filter(items, filteringText, _a) {
  ...
  return items.filter(function(item) {
    return filteringFunction(item, filteringText, fields);
  });
}
...
function propertyFilter(items, query, _a) {
  ...
  var filteringPropertiesMap = filteringProperties.reduce(function(acc, _a2) {
    var _b;
    var key = _a2.key, operators = _a2.operators, defaultOperator = _a2.defaultOperator;
  ...
  var filter2 = filteringFunction || defaultFilteringFunction2(filteringPropertiesMap);
  return items.filter(function(item) {
    return filter2(item, query);
  });
}
...
function sort(items, state) {
  ...
  var comparator = "sortingComparator" in sortingColumn ? sortingColumn.sortingComparator : sortingColumn.sortingField ? getSorter(sortingColumn.sortingField) : void 0;
  if (!comparator) {
    return items;
  }
...
// node_modules/@cloudscape-design/collection-hooks/dist/mjs/operations/paginate.js
...
function createSyncProps(options, _a, actions, collectionRef, _b) {
  ...
  return {
    collectionProps: __assign(__assign(__assign({ empty }, options.sorting ? {
...
//# sourceMappingURL=chunk-NL4VIZI4.js.map
